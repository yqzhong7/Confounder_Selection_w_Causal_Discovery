...
load(here("Simulation","2020-11-17T180306EST",sim_file_names[1]))
...
#--------load output------------####
est_file_names <- grep(".RData",list.files(here("Estimation","CD"), full.names = T), value = T)
est_df_list <- list()
mmhc_out <- list()
(s_time <- Sys.time())
for (i in 1:length(est_file_names)){
  ...
  #extract mmhc output
  mmhc_out[[i]] <- lapply(estList, function(x) as.data.frame(attributes(x)$edge_list))
  mmhc_out[[i]] <- bind_rows(mmhc_out[[i]], .id='simNo')
  #extract estimates
...
#for mmhc
mmhc_df <- bind_rows(mmhc_out, .id = 'listNo') %>%
  mutate(simNo = as.numeric(simNo)) %>%
  left_join(param_list_df, by = "simNo")
(e_time <- Sys.time() - s_time)

#combine and save into fst
write.fst(mmhc_df,  here("Analysis","mmhc_df.fst"))
write.fst(est_df_param, here("Analysis","est_df_param_cd.fst"))
...
#load mmhc
mmhc_df <- read.fst(here("Analysis","mmhc_df.fst"))
...
mmhc_df_x <- mmhc_recode(df = mmhc_df, child = 'X') 
mmhc_df_y <- mmhc_recode(df = mmhc_df, child = 'Y') 
mmhc_df_xy <- mmhc_recode(df = mmhc_df, child = c('X','Y')) 
...
save(mmhc_df_xy_sum, file = here("Analysis","summarised_output","accuracy_MMHC_default.RData"))

# mmhc_df_xy_sum %>% filter(listNo== 1 & simNo==65)

...
estimation_cd <- function(df_list, cd_constrain){
  ...
  estList <- future.apply::future_lapply(
    ...
    function(i,...) {
      ...
      #cd
      # MMHC default (alpha = 0.05, test type = 'mutual information', score = BIC, tabu = 10)
      bn <- mmhc(df, whitelist = cd_constrain$wl, blacklist = cd_constrain$bl, restrict.args = list(alpha = 0.1, test='mi'))
      plot(bn)
      edge_list <- bn$arcs %>% 
        as.data.frame() %>% 
